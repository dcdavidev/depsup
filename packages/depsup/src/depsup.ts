...
export const depsup = function (this: Command, options: DepsupOptions) {
  const { ci = false, nxVersion = 'latest', createCommit = false } = options;

  logger.info('Starting depsup...');
  ...
  if (isNxWorkspace) {
    logger.info('Nx workspace detected!');
    ...
    if (existsSync(path.join(cwd, 'migrations.json'))) {
      logger.info('Apply nx migrations...');
